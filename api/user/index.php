...
class UserAPI{
    ...
    private function login($params){
        if(is_numeric(@$_SESSION["id"])){
            die(json_encode(array("code" => 404, "data" => "you are logged to api<br>see /api/user/login/params")));       
        }
        else {
            $params = json_decode($params);
            $u_name = htmlspecialchars(trim(@$params->username));        
            $u_password = htmlspecialchars(trim(@$params->password));

            $uData = $this->database->checkCredentials($u_name,$u_password);

            if($uData != false){
                    $_SESSION["id"] = $uData["u_id"];
